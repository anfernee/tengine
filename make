...
objs/test_nginx:	objs/src/core/test_nginx.o \
	...
	$(LINK) -o objs/test_nginx \
	objs/src/core/test_nginx.o \
	objs/src/core/ngx_log.o \
	objs/src/core/ngx_palloc.o \
	objs/src/core/ngx_array.o \
	objs/src/core/ngx_list.o \
	...
	objs/src/http/modules/ngx_http_stub_status_module.o \
	objs/ngx_modules.o \
	-L/opt/local/lib -lpcre -lssl -lcrypto -lz
...
objs/src/core/test_nginx.o:	$(CORE_DEPS) \
	src/core/test_nginx.c
	$(CC) -c $(CFLAGS) $(CORE_INCS) \
		-o objs/src/core/test_nginx.o \
		src/core/test_nginx.c
...
objs/src/core/ngx_trie.o:	$(CORE_DEPS) \
	src/core/ngx_trie.c
	$(CC) -c $(CFLAGS) $(CORE_INCS) \
...
objs/src/os/unix/ngx_syslog.o:	$(CORE_DEPS) \
...
objs/src/core/ngx_dso_module.o:	$(CORE_DEPS) \
...
objs/src/http/modules/ngx_http_footer_filter_module.o:	$(CORE_DEPS) $(HTTP_DEPS) \
...
objs/src/http/modules/ngx_http_user_agent_module.o:	$(CORE_DEPS) $(HTTP_DEPS) \
...
objs/src/http/ngx_http_upstream_check_module.o:	$(CORE_DEPS) $(HTTP_DEPS) \
...
test:	objs/nginx
	TEST_NGINX_BINARY=/Users/anfernee/Code/others/tengine/objs/nginx prove -v -I /Users/anfernee/Code/others/tengine/tests/nginx-tests/nginx-tests/lib tests/nginx-tests/nginx-tests tests/nginx-tests/cases
...
install:	objs/nginx 		
	...
	test -f '$(DESTDIR)/usr/local/nginx/conf/browsers' 		|| cp conf/browsers '$(DESTDIR)/usr/local/nginx/conf'
	cp conf/browsers '$(DESTDIR)/usr/local/nginx/conf/browsers'

	test -d '$(DESTDIR)/usr/local/nginx/logs' || 		mkdir -p '$(DESTDIR)/usr/local/nginx/logs'

	test -d '$(DESTDIR)/usr/local/nginx/modules/' 		|| mkdir -p '$(DESTDIR)/usr/local/nginx/modules/'

	test -f '$(DESTDIR)/usr/local/nginx/conf/module_stubs' 		|| cp objs/module_stubs '$(DESTDIR)/usr/local/nginx/conf'
	cp objs/module_stubs '$(DESTDIR)/usr/local/nginx/conf/module_stubs'
	test -d '$(DESTDIR)/usr/local/nginx/sbin' || 		mkdir -p '$(DESTDIR)/usr/local/nginx/sbin'
	cp objs/dso_tool '$(DESTDIR)/usr/local/nginx/sbin/dso_tool'
	chmod 0755 '$(DESTDIR)/usr/local/nginx/sbin/dso_tool'
